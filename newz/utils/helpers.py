...
from newz import settings
from .logs import default_logger as log

root_dir = pathlib.Path(os.path.dirname(__file__)).parent

class FileHelper(object):
    @staticmethod
    def loadResourceFile(filename):
        path = filename if os.path.isabs(filename) else root_dir.joinpath('resources', filename).as_posix()
        try:
